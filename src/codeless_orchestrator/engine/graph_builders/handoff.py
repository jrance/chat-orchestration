...
from ..router import choose_next_agent
...
def build(
    ...
    *,
    ...
    """Build a handoff (branching) multi-agent LangGraph app.
    ...
    for aid, agent_node in agents_by_id.items():
    ...
    # Add nodes
    for aid in agents_by_id.keys():
        rt = runtimes[aid]
        graph.add_node(f"agent__{aid}", rt.agent_fn)
        graph.add_node(f"tools__{aid}", rt.tools_fn)

    # Condition factory per agent
    for aid in agents_by_id.keys():
        tools_key = f"tools__{aid}"
        out_neighbors = list(adjacency.get(aid, []))

        def _cond_factory(
            my_id: str, my_tools: str, outs: list[str]
            ...
            def _should_continue(state: EngineState) -> str:
                ...
                if not outs:
                    return END
                if len(outs) == 1:
                    return f"agent__{outs[0]}"
                # Route using LLM
                try:
                    choice = choose_next_agent(
                        messages=state.get("messages", []),
                        current_agent=my_id,
                        candidates=outs,
                        provider=runtimes[my_id].provider,
                    )
                    if choice not in outs:
                        _log.warning(
                        ...
                        return f"agent__{outs[0]}"
                    return f"agent__{choice}"
                except Exception as e:  # pragma: no cover - defensive path
                    _log.warning(
                        "Router failed (%s); defaulting to '%s'",
                        e,
                        outs[0],
                    )
                    return f"agent__{outs[0]}"
        ...
        graph.add_conditional_edges(
            f"agent__{aid}", _cond_factory(aid, tools_key, out_neighbors)
        )
        graph.add_edge(tools_key, f"agent__{aid}")

    graph.add_edge(START, f"agent__{root_id}")
    app = graph.compile()

    root = agents_by_id[root_id]
    root_rt = runtimes[root_id]
    return (
        app,
        root_id,
        root_rt.provider,
